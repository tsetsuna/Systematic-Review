...
#Seleccionar un numero de datos promedio
...
for(q in 1:length(queries)){
  ...
  #FILTAR AQUI EL NUMERO DE REGISTROS POR CADA UNO DE LOS DOMINIOS
  #Obten un numero especifico de terminos 
  print("#######################################")
  print(rdata)
  print("#######################################")
  percentageall<-70
  ...
  #if(rdata$Dominio=="Energy"){
  #  df1<-sample_n(rdata[rdata$Dominio==dominio,],round(nrow(rdata[rdata$Dominio=="Energy",])*percentageenergy/100))    
  #}else if(rdata$Dominio=="Decision Making"){
  #  df2<-sample_n(rdata[rdata$Dominio=="Decision Making",],round(nrow(rdata[rdata$Dominio=="Decision Making",])*percentageall/100))    
  #}
  
  
  df1<-sample_n(rdata[rdata$Dominio=="Energy",],round(nrow(rdata[rdata$Dominio=="Energy",])*percentageenergy/100))
  df2<-sample_n(rdata[rdata$Dominio=="Decision Making",],round(nrow(rdata[rdata$Dominio=="Decision Making",])*percentageall/100))
  df3<-sample_n(rdata[rdata$Dominio=="Data Science",],round(nrow(rdata[rdata$Dominio=="Data Science",])*percentageall/100))
  df4<-sample_n(rdata[rdata$Dominio=="Data Mining",],round(nrow(rdata[rdata$Dominio=="Data Mining",])*percentageall/100))
  df5<-sample_n(rdata[rdata$Dominio=="Machine Learning",],round(nrow(rdata[rdata$Dominio=="Machine Learning",])*percentageall/100))
  #df6<-sample_n(rdata[rdata$Dominio=="Multi Criteria Decision Making",],round(nrow(rdata[rdata$Dominio=="Multi Criteria Decision Making",])*percentageall/100))
  #df7<-sample_n(rdata[rdata$Dominio=="Data Analytics",],round(nrow(rdata[rdata$Dominio=="Data Analytics",])*percentageall/100))
  #df8<-sample_n(rdata[rdata$Dominio=="Policy",],round(nrow(rdata[rdata$Dominio=="Policy",])*percentageall/100))
  
  #Without Sample
  
 # df9<-sample_n(rdata[rdata$Dominio=="Data Visualization",],round(nrow(rdata[rdata$Dominio=="Data Visualization",])*percentageall/100))
  ...
  rdata <- rdata[order(rdata$Dominio),]
...
#Cluster 1
termsq26 <- c("Energy","Policy","Multi Criteria Decision Making","Data Analytics","Data Mining","Data Visualization")
...
#Cluster 2
termsq23 <- c("Decision Making","Policy","Machine Learning","Data Analytics","Data Visualization","Multi Criteria Decision Making")
queries23 <- c("Query23","Query23","Query23","Query23","Query23","Query23")
requerido23<- c("no","no","no","no","no","no")
df2<- data.frame(domain=c(termsq23),queryNo =c(queries23),required=c(requerido23))
dcombinations<-rbind(dcombinations,df2)

#Cluster 3
termsq26bis <- c("Energy","Data Science","Data Analytics","Data Visualization","Data Mining","Policy")
...
#Cluster 4
termsq29 <- c( "Data Science","Data Mining","Data Analytics","Policy","Decision Making","Multi Criteria Decision Making")
...
#Cluster 5
termsq30 <- c("Data Mining","Data Analytics","Policy","Decision Making","Multi Criteria Decision Making","Data Visualization")
...
#Cluster 6
termsq29bis <- c("Data Science","Data Mining","Data Analytics","Data Visualization","Policy","Machine Learning")
...
#Cluster 7
termsq30bis <- c( "Data Mining","Data Analytics","Data Visualization","Policy","Machine Learning","Multi Criteria Decision Making")
...
#Cluster 8
termsq27 <- c("Machine Learning","Data Mining","Data Analytics","Data Science","Policy","Decision Making")
queries27 <- c("Query27","Query27","Query27","Query27","Query27","Query27")
requerido27<- c("si","si","si","si","si","si")
df6<- data.frame(domain=c(termsq27),queryNo =c(queries27),required=c(requerido27))
...
v<- c("Query30","Query23","Query29","Query26","Query27")
...
for(m in 1:nrow(myqueries)){
  ...
  dcomb <- unique(as.character(unlist(domaincombinations[,1])))
...
#write.csv(processesqueries,'C:\\Mente\\Doctorado\\Stage-2019\\SystematicMap\\intermediatequeriesGOOD2.csv')
  ...
  for(k in 1:length(completeTerms)){
    ...
    if(required=="si"){#Quitar terminos en la categoria de no requeridos
      ...
      for(l in 1:nrow(excludedwords)){
        ...
        expression8 <- paste0("\\('",excludedword)
        expression8 <- paste0(expression8,"\" \\) AND")
        if(regexpr(expression1,query)[1]>-1){
          query<- gsub(pattern = expression1, replacement = "", query)
        }else if(regexpr(expression2,query)[1]>-1){
          query<- gsub(pattern = expression2, replacement = "", query)
          ...
          print(expression7)
        }else if(regexpr(expression8,query)[1]>-1){
          query<- gsub(pattern = expression8, replacement = "", query)
          print(expression8)
        }
      }
...
#######
#Esto no se ejecuta para  science direct
#######
#write.csv(processesqueriesP3,'C:\\Mente\\Doctorado\\Stage-2019\\SystematicMap\\queriesfinalessdV7.csv',quote=FALSE)
...
write.csv(processesqueriesP4,'C:\\Mente\\Doctorado\\Stage-2019\\SystematicMap\\queriesIEEE_3.csv',quote=FALSE)
write.csv(processesqueriesP4,'C:\\Users\\michiru\\Documents\\Doctorado\\Stage2019\\queriesIEEE_3.csv',quote=FALSE)
